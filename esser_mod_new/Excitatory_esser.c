...
#define nrn_init _nrn_init__Exc_esser
#define _nrn_initial _nrn_initial__Exc_esser
#define nrn_cur _nrn_cur__Exc_esser
#define _nrn_current _nrn_current__Exc_esser
#define nrn_jacob _nrn_jacob__Exc_esser
#define nrn_state _nrn_state__Exc_esser
#define _net_receive _net_receive__Exc_esser 
#define state state__Exc_esser 
...
#define tau_m _p[2]
#define e_AMPA _p[3]
#define gpeak_AMPA _p[4]
#define tau1_NMDA _p[5]
#define tau2_NMDA _p[6]
#define e_NMDA _p[7]
#define gpeak_NMDA _p[8]
#define Mg _p[9]
#define a _p[10]
#define b _p[11]
#define i _p[12]
...
extern Memb_func* memb_func;
 ...
 "tau2_NMDA", 1e-009, 1e+009,
 "tau1_NMDA", 1e-009, 1e+009,
 "tau_m", 1e-009, 1e+009,
 "tau2_AMPA", 1e-009, 1e+009,
 "tau1_AMPA", 1e-009, 1e+009,
 0,0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 Exc_esser D:/School work/TMS/nhp_tms_microcircuit/netpyne/esser_mod_new/Excitatory_esser.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t; {
    ...
    double __primary = (A_AMPA + _args[0] * factor_AMPA * gpeak_AMPA) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau1_AMPA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau1_AMPA ) - __primary );
    A_AMPA += __primary;
  } else {
 A_AMPA = A_AMPA + _args[0] * factor_AMPA * gpeak_AMPA ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B_AMPA;
    double __primary = (B_AMPA + _args[0] * factor_AMPA * gpeak_AMPA) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau2_AMPA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau2_AMPA ) - __primary );
    B_AMPA += __primary;
  } else {
 B_AMPA = B_AMPA + _args[0] * factor_AMPA * gpeak_AMPA ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = A_NMDA;
    double __primary = (A_NMDA + wratio * _args[0] * factor_NMDA * gpeak_NMDA) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau1_NMDA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau1_NMDA ) - __primary );
    A_NMDA += __primary;
  } else {
 A_NMDA = A_NMDA + wratio * _args[0] * factor_NMDA * gpeak_NMDA ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B_NMDA;
    double __primary = (B_NMDA + wratio * _args[0] * factor_NMDA * gpeak_NMDA) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau2_NMDA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau2_NMDA ) - __primary );
    B_NMDA += __primary;
  } else {
 B_NMDA = B_NMDA + wratio * _args[0] * factor_NMDA * gpeak_NMDA ;
     }
 } }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ...
   gNMDA = B_NMDA - A_NMDA ;
   iNMDA = ( gNMDA * ( v - e_NMDA ) * 1.0 / ( 1.0 + exp ( - a * v ) * ( Mg / b ) ) ) / tau_m ;
   i = iAMPA + iNMDA ;
   }
 _current += i;
